 ...
 * @since JavaFX 2.0
 */
public final class StableNumberAxis extends ValueAxis<Number> {
	...
	private DoubleProperty tickUnit = new StyleableDoubleProperty(5) {
		...
		@Override
		public CssMetaData<StableNumberAxis,Number> getCssMetaData() {
	 ...
	 */
	public StableNumberAxis(double lowerBound, double upperBound, double tickUnit) {
	 ...
	 * @param tickUnit The tick unit, i.e. space between tickmarks
	 */
	public StableNumberAxis(String axisLabel, double lowerBound, double upperBound, double tickUnit) {
		super(lowerBound, upperBound);
		setTickUnit(tickUnit);
		setLabel(axisLabel);
	}
	...
	@Override protected Object getRange() {
		double range = this.getUpperBound() - this.getLowerBound();
		
//		tickUnit.set(range/10);
//		System.out.println(tickUnit.get());
		return autoRange(this.getLowerBound(), this.getUpperBound(), 468, labelSize);
//		return new Object[]{
//				getLowerBound(),
//				getUpperBound(),
//				getTickUnit(),
...
//		};
	...
	@Override protected Object autoRange(double minValue, double maxValue, double length, double labelSize) {
		...
		// THe above "caches" the last used labelSize
	...
	private static class StyleableProperties {
		private static final CssMetaData<StableNumberAxis,Number> TICK_UNIT =
				new CssMetaData<StableNumberAxis,Number>("-fx-tick-unit",
						SizeConverter.getInstance(), 5.0) {

			@Override
			public boolean isSettable(StableNumberAxis n) {
				return n.tickUnit == null || !n.tickUnit.isBound();
			}

			@Override
			public StyleableProperty<Number> getStyleableProperty(StableNumberAxis n) {
	...
	public static class DefaultFormatter extends StringConverter<Number> {
		 ...
		 */
		public DefaultFormatter(final StableNumberAxis axis) {
		 ...
		 */
		public DefaultFormatter(StableNumberAxis axis, String prefix, String suffix) {
			this(axis);
			this.prefix = prefix;
			this.suffix = suffix;
		}
